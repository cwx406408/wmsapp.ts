<template>
  <div v-show="isShowLockScreen">
    ...
    <div class="layout-lock-screen-img" :class="{ 'layout-lock-screen-filter': isShowLockLogin }"></div>
    <div class="layout-lock-screen">
      ...
      >
        ...
        <div class="layout-lock-screen-date-top">
          ...
          <div class="layout-lock-screen-date-top-text">上划解锁</div>
        </div>
      </div>
...
import { LockScreenData } from './interface'
...
export default defineComponent({
  ...
  setup () {
    ...
    const state = reactive<LockScreenData>({
      transparency: 1,
      downClientY: 0,
      moveDifference: 0,
    ...
    // 鼠标移动
    const onMove = (move: any) : void | boolean => {
      if (state.isFlags) {
        const el = state.querySelectorEl
        const opacities = (state.transparency -= 1 / 200)
        ...
        el.setAttribute('style', `top:${state.moveDifference}px;cursor:pointer;opacity:${opacities};`)
    ...
    const initGetElement = () => {
      nextTick(() => {
        state.querySelectorEl = proxy?.proxy?.$refs.layoutLockScreenDateRef
      })
    }
